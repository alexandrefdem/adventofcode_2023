...
fn main() {
    ...
    let mut main_lightbeam_list = Vec::<LightBeam>::new();
    let mut result: Vec<u32>=Vec::new();

    for index in 0..row_limit{
        if index == 0{
            for k in 0..col_limit{
                main_lightbeam_list.push(LightBeam{
    ...
    for main_lightbeam in main_lightbeam_list{
        let mut lightbeam_list = Vec::<LightBeam>::new();
        lightbeam_list.push(main_lightbeam);
            ...
            for lb in lighbeams_loop{
                ...
                if symbol=='\\'{
                    ...
                    if lb.direction=="right" || lb.direction=="left"{     
                        ...
                        if flat_mirrors_hit.iter().any(|x| x.0==lb.row && x.1==lb.column && (x.2=="up" || x.2=="down")){
                            lb.has_finished=true
                        }
                        else{
                    ...
                    if lb.direction=="up" || lb.direction=="down"{
                        ...
                        if flat_mirrors_hit.iter().any(|x| x.0==lb.row && x.1==lb.column && (x.2=="left" || x.2=="right")){
                            lb.has_finished=true
                        }
                        else{
                            flat_mirrors_hit.push((lb.row,lb.column,lb.direction.to_string()));
                        ...
                        if flat_mirrors_hit.iter().any(|x| x.0==extra_lightbeam.row && x.1==extra_lightbeam.column && (x.2=="left" || x.2=="right")){
                            extra_lightbeam.has_finished=true
                        }
                        else{
                            flat_mirrors_hit.push((extra_lightbeam.row,extra_lightbeam.column,extra_lightbeam.direction.to_string()));
